...
trait AuthModule {
  ...
  def withStorage(ac: AuthManager): AuthManager =
    AuthManager.pure(read.fold(ac.value.map(_ <<< write[SessionId]))(Future(_)))
  ...
  def calm4AuthClient(implicit credentials: Credentials): AuthManager = {
    ...
    val signInRequest =
      Post(
        "https://calm.dhamma.org/en/users/sign_in",
        FormData(
    ...
    AuthManager.pure(for {
      ...
      session <- response
        .header[`Set-Cookie`]
        .map(_.cookie.value)
        .fold(Future.failed[String](new Exception("Login failed")))(
          Future.successful
        )
    } yield session)
  }
  ...
  implicit def credentials: Credentials = {
    val Seq(login, password, oldSessionId) =
      scala.io.Source.fromFile("data/login2").getLines().toSeq
