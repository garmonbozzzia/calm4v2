...
object CalmUriModule {
  ...
  def columnParams(n: Int): immutable.Seq[(String, String)] =
    for {
      i <- (0 to n).toList
  ...
  implicit def seq2query(seq: Seq[(String, String)]): Uri.Query =
    Uri.Query(seq.toMap)
...
trait CalmUriModule {
  ...
  implicit val searchUri: CalmUri[String @@ SearchTag] = x =>
    host
      .withPath("/en/course_applications/search")
  ...
  implicit val reflistUri: CalmUri[Int @@ ReflistTag] = appId =>
    host
  ...
  implicit val coursesUri: CalmUri[CourseListRequest] = _ =>
    host
      .withPath("/en/courses")
      .withQuery(
        columnParams(10) ++ Seq(
  ...
  implicit val conversationUri: CalmUri[ConversationRequest] = appId =>
    host
      .withPath(s"/en/course_applications/$appId/conversation_datatable")
      .withQuery(
        columnParams(8) ++ Seq(
          "order[0][column]" -> "0",
          "order[0][dir]" -> "asc",
